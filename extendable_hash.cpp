...
int main(){
    ...
    //Establezca show_messages en 0 cuando tome una 
    //entrada usando la redireccion de archivos
    show_messages = 0;

    // Establezca show_duplicate_buckets en 1 para 
    //ver todos los punteros en lugar de los únicos
...
void menu()
{
    ...
    cout<<"delete <key> <mode>      (mode: 0-Lazy / 1-Combinar buckets vacíos / 2-Combinar buckets and reducir)"<<endl;
...
void Directory::insert(int key,string value,bool reinserted){
    ...
    if(status==1){
        if(!reinserted) cout<<"Inserted key "<<key<<" in bucket "<<bucket_id(bucket_no)<<endl;
        else cout<<"Moved key "<<key<<" to bucket "<<bucket_id(bucket_no)<<endl;
    }
    else if(status==0){
        split(bucket_no);
        insert(key,value,reinserted);
    }
    else cout<<"Key "<<key<<" already exists in bucket "<<bucket_id(bucket_no)<<endl;
...
void Bucket::search(int key){
    map<int,string>::iterator it;
    it = values.find(key);
    if(it!=values.end()) {cout<<"Value = "<<it->second<<endl;}
    else  cout<<"This key does not exists"<<endl;
